...
void sl_btmesh_on_event(sl_btmesh_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_btmesh_evt_lc_server_light_onoff_updated_id:
    	app_log("Generic onoff update\r\n");
    	break;
    case sl_btmesh_evt_generic_server_state_changed_id:
    	app_log("Server state changed\r\n");
    	break;
    case sl_btmesh_evt_generic_client_server_status_id:

    	// sl_btmesh_generic_client_get(server_address, elem_index, model_id, appkey_index, type);

    	app_log("Client/server status, len: %d, data[0]: %02x, Flags: %04x, Type: %d\r\n",
    			evt->data.evt_generic_client_server_status.parameters.len,
				evt->data.evt_generic_client_server_status.parameters.data[0],
				evt->data.evt_generic_client_server_status.flags,
				evt->data.evt_generic_client_server_status.type);
    	if(evt->data.evt_generic_client_server_status.parameters.data[0])
    	{
    		is_led_on = true;
    	}
    	else
...
void app_button_press_cb(uint8_t button, uint8_t duration)
{
	...
	if (button == BUTTON_PRESS_BUTTON_0) {
		...
		//sl_btmesh_change_lightness(change_percentage)
		//sl_btmesh_generic_client_get(server_address, elem_index, model_id, appkey_index, type)
	} else {
